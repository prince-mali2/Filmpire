...
const MovieInformation = () => {
    ...
    const fetchMovieStatus = async () => {
      try {
        ...
        const favoriteResponse = await fetch(
          `https://filmpire-backend.onrender.com/favorites/${user?.sub}/${id}`
        );
        ...
        const watchlistResponse = await fetch(
          `https://filmpire-backend.onrender.com/watchlist/${user?.sub}/${id}`
        );
  ...
  const addToFavorite = async () => {
    if(!user?.sub) return alert("Login to add to favourite");
    try {
      const cleanedMovie = {
        id: data.id,
      ...
      const response = await fetch(
        `https://filmpire-backend.onrender.com/favorites`,
        {
          method: "POST",
  ...
  const removeFavorite = async () => {
    ...
    try {
      const response = await fetch(
        `https://filmpire-backend.onrender.com/favorites/${userId}/${movieId}`,
        {
          method: "DELETE",
        }
      );
  ...
  const addToWatchList = async () => {
    try {
      ...
      const response = await fetch(
        "https://filmpire-backend.onrender.com/watchlist",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const removeWatchList = async () => {
    ...
    try {
      const response = await fetch(
        `https://filmpire-backend.onrender.com/watchlist/${userId}/${movieId}`,
        {
          method: "DELETE",
        }
      );
  ...
  return (
    <ContainerSpaceAround container>
                  ...
                  character?.profile_path && (
                    ...
                    >
                      ...
                      <Typography
                        component="div"
                        color="textPrimary"
                        style={{ wordWrap: "break-word", maxWidth: "120px" }}
                      >
          ...
          <div>
            <ButtonContainer>
              <Grid size={{ xs: 12, sm: 6 }} sx={{ marginRight: "150px" }}>
                <ButtonGroup size="small" variant="outlined">
                  ...
                  <Button
                    target="_blank"
                    rel="noopener noreferrer"
                    href={`httpS://www.imdb.com/title/${data?.imdb_id}` || undefined}
                    endIcon={<MovieIcon />}
                    style={{ fontSize: "16px" }}
                  >
                    IMDB
              ...
              <Grid size={{ xs: 12, sm: 6 }}>
                <ButtonGroup size="small" variant="outlined">
                  <Button
                    onClick={!isMovieFavorited ? addToFavorite : removeFavorite}
                    endIcon={
                      isMovieFavorited ? (
                        <FavoriteBorderOutlined />
                      ) : (
                  ...
                  <Button
                    onClick={
                      !isMovieWatchListed ? addToWatchList : removeWatchList
                    }
      ...
      <Modals closeAfterTransition open={open} onClose={() => setOpen(false)}>
